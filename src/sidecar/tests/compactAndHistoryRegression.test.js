...
const { ConversationProjectionState } = require("../dist/features/conversation/ConversationProjectionState")
...
test("idle observations do not complete an active run", () => {
	...
	const status = new RuntimeStatusEventProjector({
		shouldIgnore: () => false, markFirstEvent: () => {}, activeText: () => "", hasTurnCompletionEvidence: () => false, finishTask: () => { finishes++ },
		updateTask: () => {}, broadcast: () => {}, transitionStreaming: () => {}, noteActivity: () => {}, schedulePartial: () => {}, log: () => {},
	})
	const snapshot = new AgentSnapshotEventProjector({
		bindSession: () => {}, finishTask: () => { finishes++ }, noteActivity: () => {}, activeText: () => "", hasTurnCompletionEvidence: () => false,
...
test("idle observations complete a run after the current turn produced evidence", () => {
	const finishes = []
	const status = new RuntimeStatusEventProjector({
		shouldIgnore: () => false, markFirstEvent: () => {}, activeText: () => "plain-language answer", hasTurnCompletionEvidence: () => true,
		finishTask: (sessionId, outcome, text) => finishes.push([sessionId, outcome, text]), updateTask: () => {}, broadcast: () => {},
		transitionStreaming: () => {}, noteActivity: () => {}, schedulePartial: () => {}, log: () => {},
	})
	const snapshot = new AgentSnapshotEventProjector({
		bindSession: () => {}, finishTask: (sessionId, outcome, text) => finishes.push([sessionId, outcome, text]), noteActivity: () => {},
		activeText: () => "plain-language answer", hasTurnCompletionEvidence: () => true, updateTask: () => {}, broadcast: () => {},
	})

	status.handle({ type: "status", sessionId: "status-session", status: "idle" })
	snapshot.handle({ type: "session_snapshot", sessionId: "snapshot-session", status: "idle", usage: {} })
	assert.deepEqual(finishes, [
		["status-session", "completed", "plain-language answer"],
		["snapshot-session", "completed", "plain-language answer"],
	])
})

test("starting a follow-up turn clears previous tool evidence", () => {
	const projection = new ConversationProjectionState()
	projection.rememberToolSummary("executeCommand: old turn")
	projection.beginTurn()
	assert.deepEqual(projection.recentToolSummaries(), [])
})
