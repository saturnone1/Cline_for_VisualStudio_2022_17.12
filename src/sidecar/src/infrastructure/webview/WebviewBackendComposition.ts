...
export class WebviewBackendComposition implements WebviewApplicationPort {
	...
	constructor(
		...
		this.compactSession = createContextCompactionFlow({
			...
			updateTask: () => this.taskState.update(),
			persist: () => this.schedulePersistedStateSave(),
		...
		const handlers = createWebviewFeatureHandlers({
			...
			modelCatalog: { ollamaValues: (baseUrl) => this.features.require("providerModelCatalogs").ollamaValues(baseUrl), lmStudioValues: (baseUrl) => this.features.require("providerModelCatalogs").lmStudioValues(baseUrl), refresh: (providerId, request) => this.features.require("providerModelCatalogs").refresh(providerId, request, asRecord(this.state.apiConfiguration), this.state.mode === "plan" ? "plan" : "act", this.modelContext.modelId()), askSage: (baseUrl) => this.features.require("providerModelCatalogs").askSageModels(baseUrl), openRouterKeyInfo: (apiKey) => this.features.require("providerModelCatalogs").openRouterKeyInfo(apiKey), unsupported: (key) => this.features.require("providerModelCatalogs").unsupported(key) },
			file: { host: this.host, workspaceRoot: () => this.getPrimaryWorkspaceRoot(), resolvePath: (workspaceRoot, filePath) => path.isAbsolute(filePath) ? filePath : workspaceRoot ? path.resolve(workspaceRoot, filePath) : filePath, baseName: (filePath) => path.basename(filePath), exists: (filePath) => fs.existsSync(filePath), revert: (request) => this.features.require("changeTracking").revert(request) },
