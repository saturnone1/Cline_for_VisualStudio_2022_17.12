...
import { formatEmptyModelResponseForUi, formatRunRetryNoticeForUi, formatSdkErrorForUi, isSessionNotFoundError, stringify } from "./RuntimeErrorFormatter"
...
import { createRuntimeErrorRecoveryPolicy } from "../../application/services/RuntimeErrorRecoveryPolicy"
...
export class WebviewBackendComposition implements WebviewApplicationPort {
	...
	private readonly runtimeErrorRecovery = createRuntimeErrorRecoveryPolicy({
		maxAttempts: () => readPositiveIntEnv("VSCLINE_MAX_RUN_ERROR_RETRIES", 2),
	...
	constructor(
		...
		this.sendUserMessage = new ChatFlows.SendUserMessageFlow({
			...
			projection: {
					addUserMessage: (text, images, files) => { this.conversationProjection.beginTurn(); this.conversationMessages.removeTerminalAsks(); this.conversationMessages.removeAsks("followup"); const message = this.conversationMessages.add({ type: "say", say: "user_feedback", text, images, files }); this.foldedProgressProjector.beginReasoning(); return message },
