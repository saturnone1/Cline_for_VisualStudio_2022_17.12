	...
	constructor(
		...
		this.taskSession = new TaskSessionCoordinator({
			...
			rebindSnapshot: (previousTaskId, sessionId) => {
				const snapshot = this.getTaskSnapshot(previousTaskId)
				if (!snapshot) return
				this.forgetTaskSnapshot(previousTaskId)
		...
		this.semanticEvents = new AgentEventDispatcher({ bindSession: (sessionId) => this.taskSession.bindSession(sessionId), projectText: (event) => this.agentTextEvents.handle(event), projectTool: (event) => this.agentToolEvents.handle(event), projectLifecycle: (event) => this.agentLifecycleEvents.handle(event), updateTask: () => this.updateCurrentTaskItem(), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) } })
		...
		this.agentSnapshotEvents = new AgentSnapshotEventProjector({ bindSession: (sessionId) => this.taskSession.bindSession(sessionId), finishTask: (sessionId, status, text) => this.finishSdkTask(sessionId, status, text), noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), activeText: () => this.getActivePartialText(), updateTask: (updates) => this.updateCurrentTaskItem(updates), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) } })
