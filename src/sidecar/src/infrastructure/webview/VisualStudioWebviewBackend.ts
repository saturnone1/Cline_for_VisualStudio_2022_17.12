...
export class VisualStudioWebviewBackend implements WebviewApplicationPort {
	...
	constructor(
		...
		this.agentLifecycleEvents = new AgentLifecycleEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), clearReasoning: () => this.conversationActivity.clearReasoning(), finishToolActivity: () => this.conversationRuntime.finishToolActivity(), finishProgress: () => this.foldedProgressProjector.finish(), finalizePartial: () => this.partialTextProjector.finalize(), addText: (text) => this.addMessage({ type: "say", say: "text", text }), addError: (text) => this.addMessage({ type: "say", say: "error", text }), finishTask: (sessionId, status, text) => this.taskCompletion.finish(sessionId, status, text), updateUsage: (usage) => this.taskState.update(usage), hasCompletion: () => this.taskCompletion.hasCompletionAfterLastUser(), activePartialText: () => this.partialTextProjector.activeText(), hasAssistantAfterUser: () => this.taskCompletion.hasAssistantAfterLastUser(), log: (event, details) => this.logger.log("sidecar", event, details), formatError: (error) => formatProviderErrorForTranscript(error, this.getUiLanguage()), markErrorLatency: (sessionId, error) => this.runtimeMonitoring.markError(sessionId, error) })
