...
async function normalizeSdkImageInputs(images: string[]) {
	return (await Promise.all(images.map((image) => normalizeSdkImageInput(image)))).filter(Boolean)
}

async function normalizeSdkImageInput(image: string) {
...
async function tryCreateImageDataUri(filePath: string) {
	try {
...
function loadInitialState(persisted: Record<string, unknown> | null) {
...
function createPersistedStateSnapshot(state: ReturnType<typeof createInitialState>): Record<string, unknown> {
	return {
