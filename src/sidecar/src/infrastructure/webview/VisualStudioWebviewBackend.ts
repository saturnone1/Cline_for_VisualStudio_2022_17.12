...
export class VisualStudioWebviewBackend implements WebviewApplicationPort {
	...
	constructor(
		...
		this.taskTranscriptHydrator = new TaskTranscriptHydrator({
			...
			applyCompleted: (taskId, task, messages) => { this.runtimeMonitoring.clearTaskActivity(); this.runtimeMonitoring.clearPartialIdle(); this.clearReasoningStatus(); this.conversationProjection.activePartialTextTs = null; this.conversationProjection.activeReasoningTextTs = null; this.conversationProjection.activeToolActivityTs = null; this.conversationProjection.activeAssistantTextBuffer = ""; this.state.currentTaskItem = task; this.state.clineMessages = messages; this.finalizeOpenPartialMessages(); this.taskCompletion.addMarker("completed"); this.taskState.update(); this.taskState.remember(taskId, task, this.state.clineMessages); this.schedulePersistedStateSave() },
		...
		this.agentLifecycleEvents = new AgentLifecycleEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), clearReasoning: () => this.clearReasoningStatus(), finishToolActivity: () => this.finishActiveToolActivity(), finishProgress: () => this.finishFoldedReasoningText(), finalizePartial: () => this.finalizeActivePartialText(), addText: (text) => this.addMessage({ type: "say", say: "text", text }), addError: (text) => this.addMessage({ type: "say", say: "error", text }), finishTask: (sessionId, status, text) => this.taskCompletion.finish(sessionId, status, text), updateUsage: (usage) => this.taskState.update(usage), hasCompletion: () => this.taskCompletion.hasCompletionAfterLastUser(), activePartialText: () => this.getActivePartialText(), hasAssistantAfterUser: () => this.taskCompletion.hasAssistantAfterLastUser(), log: (event, details) => this.logger.log("sidecar", event, details), formatError: (error) => formatProviderErrorForTranscript(error, this.getUiLanguage()), markErrorLatency: (sessionId, error) => this.runtimeMonitoring.markError(sessionId, error) })
		...
		this.agentSnapshotEvents = new AgentSnapshotEventProjector({ bindSession: (sessionId) => this.taskSession.bindSession(sessionId), finishTask: (sessionId, status, text) => this.taskCompletion.finish(sessionId, status, text), noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), activeText: () => this.getActivePartialText(), updateTask: (updates) => this.taskState.update(updates), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) } })
		this.agentChunkEvents = new AgentChunkEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), noteQuietActivity: (reason) => this.runtimeMonitoring.noteQuietActivity(reason), finishTask: (status, text) => this.taskCompletion.finish(this.clineSdk?.status.activeSessionId || String(this.state.currentTaskItem?.id || ""), status, text), addMessage: (message) => { this.addMessage(message) }, recordTool: (text) => this.recordToolActivity("tool", text), foldReasoning: (text) => this.upsertFoldedReasoningText(text), updateTask: () => this.taskState.update(), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) }, schedulePartial: () => this.runtimeMonitoring.schedulePartialBroadcast(), recentTexts: () => this.state.clineMessages.slice(-3).map((message) => getString(message, "text")), commandOutputLimit: () => readPositiveIntEnv("VSCLINE_COMMAND_OUTPUT_CHARS", 12000), agentTranscriptLimit: () => readPositiveIntEnv("VSCLINE_AGENT_TRANSCRIPT_CHARS", 12000), logSkipped: (chunk) => this.logger.log("sidecar", "sdkAgentChunkSkippedForUi", summarizeAgentChunkForLog(chunk)) })
