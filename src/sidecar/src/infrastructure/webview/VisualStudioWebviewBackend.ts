...
export class VisualStudioWebviewBackend implements WebviewApplicationPort {
	...
	constructor(
		...
		this.taskCompletion = new TaskCompletionProjector({ messages: () => this.state.clineMessages, transition: (status, source) => { this.taskSession.transition(status, source) }, clearFinishStatus: () => { this.runtimeMonitoring.clearTaskActivity(); this.runtimeMonitoring.clearPartialIdle(); this.clearReasoningStatus() }, finishProgress: () => { this.partialTextProjector.finalize(); this.finishActiveToolActivity(); this.foldedProgressProjector.finish() }, prepareAssistant: () => { this.runtimeMonitoring.clearTaskActivity(); this.runtimeMonitoring.clearPartialIdle(); this.partialTextProjector.finalize(); this.finishActiveToolActivity(); this.foldedProgressProjector.finish() }, activeText: () => this.partialTextProjector.activeText(), addMessage: (message) => { this.addMessage(message) }, markAssistantLatency: (length) => this.runtimeMonitoring.markFirstAssistant(this.getCurrentSessionId(), length), finalizeOpenPartial: () => this.finalizeOpenPartialMessages(), lastActivityReason: () => this.taskActivity?.reason || "", runCompleteHook: (context) => { void this.runLifecycleHooks("TaskComplete", context) }, persist: () => this.stateStore.save(createPersistedStateSnapshot(this.state)), language: () => this.getUiLanguage(), recentToolSummaries: () => this.conversationProjection.recentToolSummaries(5), log: (event, details) => this.logger.log("sidecar", event, details) })
		...
		this.agentChunkEvents = new AgentChunkEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), noteQuietActivity: (reason) => this.runtimeMonitoring.noteQuietActivity(reason), finishTask: (status, text) => this.taskCompletion.finish(this.clineSdk?.status.activeSessionId || String(this.state.currentTaskItem?.id || ""), status, text), addMessage: (message) => { this.addMessage(message) }, recordTool: (text) => this.recordToolActivity("tool", text), foldReasoning: (text) => this.foldedProgressProjector.upsertReasoning(text), updateTask: () => this.taskState.update(), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) }, schedulePartial: () => this.runtimeMonitoring.schedulePartialBroadcast(), recentTexts: () => this.state.clineMessages.slice(-3).map((message) => getString(message, "text")), commandOutputLimit: () => readPositiveIntEnv("VSCLINE_COMMAND_OUTPUT_CHARS", 12000), agentTranscriptLimit: () => readPositiveIntEnv("VSCLINE_AGENT_TRANSCRIPT_CHARS", 12000), logSkipped: (chunk) => this.logger.log("sidecar", "sdkAgentChunkSkippedForUi", summarizeAgentChunkForLog(chunk)) })
	...
	private recordToolActivity(tool: string, text: string) {
		...
		this.foldedProgressProjector.upsertActivity(groupedText)
	}
	...
	private upsertAssistantTextFromEvent(accumulated: string, delta: string) {
		...
		if (shouldFoldTextContentAsReasoning(normalized)) {
			this.foldedProgressProjector.upsertReasoning(normalized)
			return
		}
		...
		this.foldedProgressProjector.finish()
		this.partialTextProjector.upsert(normalized)
	}
	...
	private completeAssistantText(text: string) {
		...
		if (timestamp) {
			this.conversationMessages.upsert(timestamp, { type: "say", say: "text", text, partial: false })
