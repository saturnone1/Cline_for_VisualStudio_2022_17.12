...
export class VisualStudioWebviewBackend implements WebviewApplicationPort {
	...
	constructor(
		...
		this.sendUserMessage = new SendUserMessageFlow({ hasPendingApproval: () => this.approvals.hasPending, hasPendingQuestion: () => Boolean(this.pendingQuestion), clearPending: () => { this.approvals.clear({ approved: false, reason: "Superseded by resumed chat message." }); this.pendingQuestion?.resolve(""); this.pendingQuestion = null }, startNewTask: (input) => this.startNewTask({ text: input.prompt, images: input.images, files: input.files }, { broadcast: true, requestId: input.requestId }), startLatency: (requestId, sessionId, textLength) => this.runtimeMonitoring.startLatency(requestId, "askResponse", sessionId, textLength), transitionStarting: () => { this.transitionTask("starting", "send-response") }, projectUserMessage: (text) => { this.removeTerminalAskMessages(); const message = this.addMessage({ type: "say", say: "user_feedback", text }); this.foldedProgressProjector.beginReasoning(); return message }, showPreparing: () => this.upsertFoldedReasoningText(this.state.uiLanguage === "en" ? "Preparing response." : "응답을 준비하는 중입니다."), persist: () => this.schedulePersistedStateSave(), publishPartial: (message) => this.sendPartialMessage(message !== null && typeof message === "object" && !Array.isArray(message) ? message as Record<string, unknown> : undefined), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) }, normalizeImages: (images) => normalizeSdkImageInputs(images), runHook: (context) => { void this.runLifecycleHooks("UserPromptSubmit", context) }, nextGeneration: () => ++this.sdkRunGeneration, currentGeneration: () => this.sdkRunGeneration, send: (sessionId, command, textLength) => this.sendOrResumeSdkSession(sessionId, command, textLength), resultSessionId: (result, fallback) => getString(asRecord(result), "sessionId") || fallback, complete: (result, sessionId, generation) => this.completeFromSdkResult(result, sessionId, "send", generation), recover: (sessionId, generation, error) => this.recoverFromSdkRunError(sessionId, "send", generation, error), log: (event, details) => this.logger.log("sidecar", event, details) })
		...
		this.taskTranscriptHydrator = new TaskTranscriptHydrator({
			...
			applyCompleted: (taskId, task, messages) => { this.runtimeMonitoring.clearTaskActivity(); this.runtimeMonitoring.clearPartialIdle(); this.clearReasoningStatus(); this.conversationProjection.activePartialTextTs = null; this.conversationProjection.activeReasoningTextTs = null; this.conversationProjection.activeToolActivityTs = null; this.conversationProjection.activeAssistantTextBuffer = ""; this.state.currentTaskItem = task; this.state.clineMessages = messages; this.finalizeOpenPartialMessages(); this.addCompletionResultMarker("completed"); this.updateCurrentTaskItem(); this.rememberTaskSnapshot(taskId, task, this.state.clineMessages); this.schedulePersistedStateSave() },
		...
		this.agentAuxiliaryEvents = new AgentAuxiliaryEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), addMessage: (message) => { this.addMessage(message) }, updateTask: () => this.updateCurrentTaskItem(), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) }, log: (event, details) => this.logger.log("sidecar", event, details) })
		this.agentSnapshotEvents = new AgentSnapshotEventProjector({ bindSession: (sessionId) => this.bindCurrentTaskToSession(sessionId), finishTask: (sessionId, status, text) => this.finishSdkTask(sessionId, status, text), noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), activeText: () => this.getActivePartialText(), updateTask: (updates) => this.updateCurrentTaskItem(updates), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) } })
		this.agentChunkEvents = new AgentChunkEventProjector({ noteActivity: (reason) => this.runtimeMonitoring.noteActivity(reason), noteQuietActivity: (reason) => this.runtimeMonitoring.noteQuietActivity(reason), finishTask: (status, text) => this.finishSdkTask(this.clineSdk?.status.activeSessionId || String(this.state.currentTaskItem?.id || ""), status, text), addMessage: (message) => { this.addMessage(message) }, recordTool: (text) => this.recordToolActivity("tool", text), foldReasoning: (text) => this.upsertFoldedReasoningText(text), updateTask: () => this.updateCurrentTaskItem(), broadcast: () => { this.broadcastState().catch((error) => console.error(error)) }, schedulePartial: () => this.runtimeMonitoring.schedulePartialBroadcast(), recentTexts: () => this.state.clineMessages.slice(-3).map((message) => getString(message, "text")), commandOutputLimit: () => readPositiveIntEnv("VSCLINE_COMMAND_OUTPUT_CHARS", 12000), agentTranscriptLimit: () => readPositiveIntEnv("VSCLINE_AGENT_TRANSCRIPT_CHARS", 12000), logSkipped: (chunk) => this.logger.log("sidecar", "sdkAgentChunkSkippedForUi", summarizeAgentChunkForLog(chunk)) })
	...
	private bindCurrentTaskToSession(sessionId: string) {
		...
		this.runtimeMonitoring.rebindLatency(currentTaskId, sessionId)
