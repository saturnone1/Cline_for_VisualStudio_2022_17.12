...
export class VisualStudioWebviewBackend implements WebviewApplicationPort {
	...
	constructor(
		...
		this.streamPublisher = new WebviewStreamPublisher(this.transport, this.logger, () => this.serializeState())
		...
		this.taskRpc = new TaskRpcHandler({ hasPendingQuestion: () => Boolean(this.pendingQuestion), hasCurrentTask: () => Boolean(this.state.currentTaskItem), start: (request, requestId) => this.taskPrompts.start(request, { broadcast: true, requestId }), respond: (request, requestId) => this.taskPrompts.respond(request, requestId), compact: (requestId) => this.compactSession.execute(requestId), cancel: () => this.cancelTaskFlow.execute(), clear: () => this.clearTaskHandler.execute(), refreshHistory: (source) => this.taskHistorySync.refreshInBackground(source), history: () => this.state.taskHistory, show: (taskId) => this.taskTranscriptHydrator.show(taskId), delete: (taskIds) => this.taskHistoryCommands.delete(taskIds), deleteAll: () => this.taskHistoryCommands.deleteAll(), toggleFavorite: (taskId, isFavorited) => this.taskHistoryCommands.toggleFavorite(taskId, isFavorited), broadcast: () => this.broadcastState() })
	...
	private async handleGrpcRequest(request: GrpcRequest) {
		...
		try {
			...
			this.conversationMessages.add({ type: "say", say: "error", text: message })
			this.taskState.update()
			await this.broadcastState()
