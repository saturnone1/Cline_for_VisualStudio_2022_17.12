...
export class AgentSdkConfigBuilder {
	...
	async build(cwd: string, sessionId?: string) {
		...
		return { providerId: sdkProviderId, modelId, sessionId: sessionId || undefined, apiKey, baseUrl: sdkBaseUrl || undefined, cwd, workspaceRoot: cwd, mode, enableTools: true, enableSpawnAgent: subagentsEnabled, enableAgentTeams: subagentsEnabled, ...(maxIterations ? { maxIterations } : {}), ...(maxParallelToolCalls ? { maxParallelToolCalls } : {}), ...(maxTokensPerTurn ? { maxTokensPerTurn } : {}), ...(apiTimeoutMs ? { apiTimeoutMs } : {}), thinking, reasoningEffort, providerConfig: { ...(maxTokensPerTurn ? { maxTokens: maxTokensPerTurn } : {}), ...(apiTimeoutMs ? { timeout: apiTimeoutMs } : {}), ...(Object.keys(oauthCredentials).length > 0 ? { oauthCredentials } : {}), reasoning: { enabled: thinking, effort: reasoningEffort } }, checkpoint: { enabled: state.enableCheckpointsSetting !== false }, compaction: { enabled: state.useAutoCondense === true, strategy: "basic", thresholdRatio: 0.9, ...(contextWindowTokens ? { maxInputTokens: contextWindowTokens } : {}) }, ...(execution ? { execution } : {}), preferredLanguage, systemPrompt }
	}
}
...
function readPositiveIntEnv(name: string, fallback: number) { const value = Number.parseInt(process.env[name] || "", 10); return Number.isFinite(value) && value > 0 ? value : fallback }
