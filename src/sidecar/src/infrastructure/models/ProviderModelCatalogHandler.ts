...
export class ProviderModelCatalogHandler {
	...
	async refresh(providerId: string, request: ProviderCatalogRequest, configuration: Record<string, unknown>, mode: "plan" | "act", fallbackModelId: string, signal?: AbortSignal) {
		...
		if (provider === "ollama") {
			const ids = await getOllamaModels(baseUrl, signal)
		...
		const result = await getOpenAiCompatibleModels(baseUrl, apiKey, signal)
	...
	async ollamaValues(baseUrl: string, signal?: AbortSignal) {
		const values = await getOllamaModels(baseUrl, signal)
		if (values.length) this.applyDefaultOllamaModel(values[0])
		return { values }
	}

	async lmStudioValues(baseUrl: string, signal?: AbortSignal) {
		const result = await getOpenAiCompatibleModels(baseUrl || "http://localhost:1234/v1", "", signal)
		return {
	...
	async askSageModels(baseUrl: string, signal?: AbortSignal) {
		try {
			const { response, text } = await fetchBoundedText(`${baseUrl.replace(/\/+$/, "")}/get-models`, {}, catalogFetchOptions(signal))
	...
	async openRouterKeyInfo(apiKey: string, signal?: AbortSignal) {
		if (!apiKey) return { data: null, error: "OpenRouter API key is required." }
		try {
			const { response, text } = await fetchBoundedText("https://openrouter.ai/api/v1/key", { headers: { Authorization: `Bearer ${apiKey}` } }, catalogFetchOptions(signal))
			if (!response.ok) return { data: null, error: `OpenRouter key endpoint returned HTTP ${response.status}.` }
			const data = asRecord(parseRecord(text).data)
...
function catalogFetchOptions(signal?: AbortSignal) {
	return {
		timeoutMs: readBoundedPositiveIntEnv("VSCLINE_MODEL_CATALOG_TIMEOUT_MS", RUNTIME_DEFAULTS.metadataRequestTimeoutMs, 500, 120_000),
