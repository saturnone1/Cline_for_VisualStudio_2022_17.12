...
type Callbacks = Readonly<{
	...
	activeText: () => string
	hasTurnCompletionEvidence: () => boolean
...
export class AgentSnapshotEventProjector {
	...
	handle(event: SessionSnapshotRuntimeEvent) {
		...
		if (event.status === "idle" && this.callbacks.hasTurnCompletionEvidence()) this.callbacks.finishTask(event.sessionId, "completed", this.callbacks.activeText())
		else if (event.status && !ACTIVE_STATUSES.has(event.status)) this.callbacks.finishTask(event.sessionId, event.status, this.callbacks.activeText())
