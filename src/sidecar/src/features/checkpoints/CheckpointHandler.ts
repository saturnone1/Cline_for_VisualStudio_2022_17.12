...
export type CheckpointTargetRequest = Readonly<{ checkpointRunCount?: number; runCount?: number; messageTs?: number; restoreType?: string }>

export class CheckpointHandler {
	constructor(private readonly agentEngine: AgentEnginePort) {}

	async restore(request: CheckpointTargetRequest, context: RestoreContext) {
		const checkpointRunCount = targetRunCount(request, context.messages)
...
function targetRunCount(request: CheckpointTargetRequest, messages: readonly Record<string, unknown>[], messageTs = request.messageTs) { return request.checkpointRunCount ?? request.runCount ?? findCheckpointRunCount(messages, messageTs) }
