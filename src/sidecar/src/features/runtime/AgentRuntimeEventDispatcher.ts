...
export class AgentRuntimeEventDispatcher {
	...
	handle(event: AgentRuntimeEvent) {
		...
		if (event.type === "agent_event") {
			...
			this.transitionStreaming(eventType)
		...
		if (event.type === "vscline_file_changed") {
			if (!event.sessionId) {
				this.callbacks.log("ignoredUnscopedWorkspaceChange", { filePath: event.change.filePath })
				return
			}
			if (this.ignore(event.sessionId, "ignoredSdkWorkspaceChange")) return
		...
		if (event.type === "unknown") {
			...
			if (!sessionId) {
				this.callbacks.log("ignoredUnscopedSdkEvent", { eventType })
				return
			}
			if (!this.ignore(sessionId, "ignoredUnknownSdkEvent")) this.transitionStreaming(eventType)
		}
	}
	...
	private transitionStreaming(eventType: string) {
		if (eventType && eventType !== "vscline_file_changed" && eventType !== "status" && eventType !== "ended") {
			this.callbacks.transitionStreaming(`sdk:${eventType}`)
		}
	}
}
