...
export class CompactSessionFlow {
	...
	async execute(requestId: string) {
		...
		const command: SendMessageCommand = { sessionId, prompt, mode: this.callbacks.mode(), delivery: "steer" }
		const generation = this.callbacks.nextGeneration()
		try {
			const result = await this.callbacks.send(sessionId, command, prompt.length)
			await this.callbacks.complete(result, this.callbacks.resultSessionId(result, sessionId), generation)
		} catch (error) {
