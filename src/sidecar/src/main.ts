...
import { flushInteractionLog, interactionLogger, logInteraction } from "./infrastructure/diagnostics/InteractionLog"
...
import { JsonRpcWebviewTransport, type JsonRpcConnection } from "./infrastructure/transport/JsonRpcConnection"
...
const server = net.createServer((socket) => {
	...
	const hostProvider = VisualStudioHostProvider.create(connection)
	const webviewBackend = new VisualStudioWebviewBackend(hostProvider, new JsonRpcWebviewTransport(connection), interactionLogger, stateStore)
	const webviewRouter = new VisualStudioWebviewController(webviewBackend)
	...
	webviewBackend.setClineSdk(clineSdk)
	webviewBackend.setTaskSessionUseCase(new TaskSessionUseCase(clineSdk))
	webviewBackend.setMcpUseCase(new McpUseCase(clineSdk))
...
function handleMessage(
	...
	webviewRouter: VisualStudioWebviewController,
	line: string,
) {
