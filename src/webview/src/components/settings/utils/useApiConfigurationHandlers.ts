...
import { convertApiConfigurationToProto } from "@shared/protoConversions/models/apiConfigurationConversion"
	...
	const handleModeFieldsChange = async <T extends Record<string, any>>(
		...
		if (planActSeparateModelsSetting) {
			...
			Object.entries(fieldPairs).forEach(([key, { plan, act }]) => {
				...
				;(updates as Record<string, any>)[String(targetField)] = values[key]
			})
			...
			const updates: Partial<ApiConfiguration> = {}
			Object.entries(fieldPairs).forEach(([key, { plan, act }]) => {
				;(updates as Record<string, any>)[String(plan)] = values[key]
				;(updates as Record<string, any>)[String(act)] = values[key]
			})
			await handleFieldsChange(updates)
		}
	}
