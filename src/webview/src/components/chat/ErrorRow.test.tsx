...
// Mock ClineError
vi.mock("@shared/services/error/ClineError", () => ({
	ClineError: {
		parse: vi.fn(),
	},
	ClineErrorType: {
...
describe("ErrorRow", () => {
	...
	describe("API error handling", () => {
		it("renders credit limit error when balance error is detected", async () => {
			...
			const { ClineError } = await import("@shared/services/error/ClineError")
		...
		it("renders friendly logged-out message and sign in button when user is not signed in", async () => {
			...
			expect(screen.getByText("Sign in to LIG VS")).toBeInTheDocument()
		})
