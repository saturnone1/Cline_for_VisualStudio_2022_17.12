...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	if (!data) {
		return <div className="text-foreground opacity-80">{t("subagent.unavailable")}</div>
	}
	...
	const title = singular ? t("subagent.wantsOne") : t("subagent.wantsMany")
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const statsText = `${formatCount(entry.toolCalls)} tools called · ${formatCount(entry.contextTokens)} tokens`
