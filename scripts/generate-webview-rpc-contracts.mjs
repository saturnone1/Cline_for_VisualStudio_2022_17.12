...
import { renderWebviewRpcTypescript } from "./render-webview-rpc-typescript.mjs"
...
if (bootstrap) {
	manifest = { $schema: "./webview-rpc.schema.json", schemaVersion: 3, protocolVersion: 1, shapes: {}, operations: discovered }
...
function validateManifestShape(value) {
	if (value?.schemaVersion !== 3 || !Number.isInteger(value?.protocolVersion) || !isObject(value?.shapes) || !Array.isArray(value?.operations)) fail("Invalid WebView RPC manifest header.")
	for (const [shapeName, shape] of Object.entries(value.shapes)) {
		...
		for (const [fieldName, field] of Object.entries(shape.fields)) {
			...
			if (field.shape !== undefined && (!["object", "objectArray"].includes(field.type) || typeof field.shape !== "string" || !value.shapes[field.shape])) fail(`Invalid nested RPC payload shape: ${shapeName}.${fieldName}`)
			if (field.values !== undefined && (field.type !== "string" || !Array.isArray(field.values) || field.values.length === 0 || field.values.some((item) => typeof item !== "string"))) fail(`Invalid RPC payload enum: ${shapeName}.${fieldName}`)
		}
...
function generatedOutputs(value) {
	...
	const sidecarRouteValues = [...new Set(value.operations.filter((item) => item.sidecar).map((item) => item.route))]
	const payloadTypes = Object.entries(value.shapes).map(([name, shape]) => `\t${JSON.stringify(name)}: {\n${Object.entries(shape.fields).map(([fieldName, rule]) => `\t\t${JSON.stringify(fieldName)}${rule.required ? "" : "?"}: ${typescriptPayloadType(rule)}`).join("\n")}${shape.allowAdditionalFields ? `${Object.keys(shape.fields).length ? "\n" : ""}\t\t[key: string]: unknown` : ""}\n\t}`).join("\n")
	...
	const renderedTsContent = renderWebviewRpcTypescript(value, operations, sidecarRouteValues)
	...
	return [
		[path.join(repoRoot, "src/webview/src/services/generated/WebviewRpcContract.ts"), renderedTsContent],
		[path.join(repoRoot, "src/sidecar/src/application/dto/generated/WebviewRpcContract.ts"), renderedTsContent],
...
function fixtureForShape(shape) {
	if (!shape) return {}
	return Object.fromEntries(Object.entries(shape.fields).filter(([, rule]) => rule.required).map(([field, rule]) => [field, rule.values?.[0] ?? (rule.type === "string" ? "fixture" : rule.type === "number" ? 0 : rule.type === "boolean" ? false : rule.type.endsWith("Array") || rule.type === "array" ? [] : {})]))
}

function typescriptPayloadType(rule) {
