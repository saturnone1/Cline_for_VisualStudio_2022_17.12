...
$hostTests = Join-Path $root "tests\extension\VsClineAgent.Host.Tests.csproj"
...
# The host test project is intentionally outside the product solution. Restore
# it separately so Verify-Repository.ps1 also works on an air-gapped machine.
dotnet restore $hostTests --packages $PackagesFolder --no-cache
if ($LASTEXITCODE -ne 0) {
    throw "Host test package restore failed with exit code $LASTEXITCODE."
}
