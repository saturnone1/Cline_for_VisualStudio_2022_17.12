...
- WebView history preview now also asks `TaskService.getTaskHistory` on mount instead of relying only on the first state snapshot, so the first screen can show saved sessions even before a full sidecar state refresh arrives.
