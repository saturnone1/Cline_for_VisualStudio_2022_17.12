...
- Short plain `stream: "agent"` fragments are treated as SDK/model reasoning noise unless they are explicit tool transcript rows. User-visible assistant text should arrive through SDK `content_*` events, which avoids one-token-per-line reasoning output in the WebView.
- Structured `content_start`, `content_update`, `content_delta`, and `content_end` records embedded inside raw `stream: "agent"` chunks are not rendered from the chunk path. Those records are handled by the dedicated SDK `agent_event` path instead, preventing duplicate assistant text and internal reasoning narration from appearing before the final transcript row.
