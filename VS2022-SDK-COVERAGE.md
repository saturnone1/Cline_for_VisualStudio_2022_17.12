...
- Structured `content_start`, `content_update`, `content_delta`, and `content_end` records embedded inside raw `stream: "agent"` chunks are split by content type. User-visible text/tool records are normalized into transcript rows, tool and command rows are collapsed by default, and reasoning records are collapsed so final assistant Markdown remains visible without exposing internal token streams.
