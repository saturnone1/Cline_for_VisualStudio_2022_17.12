...
- `VisualStudioWebviewBackend.ts` has been reduced below 1,700 lines and no longer owns a raw unary RPC switch, runtime monitoring policy, task/session lifecycle policy, or delayed state-stream hydration policy, but remains a transitional composition facade for a small set of runtime state and conversation adapters;
